...
std::atomic<uint32_t> Gnss::sLastNumSvs{0};
std::atomic<bool> Gnss::sHaveSvReport{false};
...
void Gnss::locationCb(GpsLocation* location) {
    ...
    if ((location->flags & GPS_LOCATION_HAS_LAT_LONG) == 0) {
        ALOGW("%s: drop location without lat/lon flags=0x%x", __func__, location->flags);
        return;
    }
    if (location->latitude == 0.0 && location->longitude == 0.0) {
        ALOGW("%s: drop null-island location", __func__);
        return;
    }
    if (sHaveSvReport.load() && sLastNumSvs.load() == 0) {
        ALOGE("%s: drop location with 0 SVs (not a GNSS fix)", __func__);
        return;
    }
...
void Gnss::gnssSvStatusCb(GnssSvStatus* status) {
    ...
    sHaveSvReport.store(true);
    sLastNumSvs.store(svStatus.numSvs);
...
void Gnss::nmeaCb(GpsUtcTime timestamp, const char* nmea, int length) {
    ...
    // CAF loc_eng_nmea_put_checksum (msm8994) skips '$' then returns
    // payload+checksum, one short of the C string. AOSP
    // hidl_string::setToExternal(nmea, length) does
    // LOG_ALWAYS_FATAL_IF(data[size] != '\0') and SIGABRTs because
    // data[length] is '\n'. MPSS QMI NMEA may also omit the terminator.
    // Copy into an owned buffer. Never setToExternal on the CAF pointer.
    // Never use CAF's length as the HIDL size.
    static constexpr size_t kMaxNmea = 256;  // CAF NMEA_SENTENCE_MAX_LENGTH is 200
    char owned[kMaxNmea];
    const size_t cap = kMaxNmea - 1;
    size_t limit = cap;
    if (length > 0) {
        // One extra byte covers the omitted last character ('\n').
        size_t want = static_cast<size_t>(length) + 1;
        if (want < cap) {
            limit = want;
        }
    }
    size_t n = 0;
    while (n < limit && nmea[n] != '\0') {
        owned[n] = nmea[n];
        ++n;
    }
    owned[n] = '\0';
    ...
    android::hardware::hidl_string nmeaString(owned, n);
...
void Gnss::setCapabilitiesCb(uint32_t capabilities) {
    ...
    // MSA needs a RIL SUPL PDN. Do not advertise it.
    capabilities &= ~static_cast<uint32_t>(GPS_CAPABILITY_MSA);
...
Return<bool> Gnss::setPositionMode(IGnss::GnssPositionMode mode,
    ...
    GpsPositionMode gpsMode = static_cast<GpsPositionMode>(mode);
    if (gpsMode == GPS_POSITION_MODE_MS_ASSISTED) {
        ALOGI("%s: MSA requested, forcing STANDALONE (no RIL)", __func__);
        gpsMode = GPS_POSITION_MODE_STANDALONE;
    }

    return (mGnssIface->set_position_mode(gpsMode,
