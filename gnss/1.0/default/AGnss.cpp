...
AGnss::AGnss(const AGpsInterface* aGpsIface) : mAGnssIface(aGpsIface) {
    ...
    sAGnssIface = aGpsIface;
}

bool AGnss::openWlanAgps() {
    if (sAGnssIface == nullptr || !talkmanWlanIsUp()) {
        return false;
    }
    int rc = sAGnssIface->data_conn_open_with_apn_ip_type(
            "wlan0", static_cast<uint16_t>(IAGnss::ApnIpType::IPV4V6));
    if (rc != 0) {
        ALOGE("%s: wlan0 data_conn_open failed %d", __func__, rc);
        return false;
    }
    sOpenedWlanAgps = true;
    ALOGI("%s: AGPS data conn on wlan0 (no RIL)", __func__);
    return true;
}

AGnss::~AGnss() {
    sThreadFuncArgsList.clear();
    sOpenedWlanAgps = false;
    sAGnssIface = nullptr;
...
void AGnss::statusCb(AGpsStatus* status) {
    ...
    if (status->status == GPS_REQUEST_AGPS_DATA_CONN) {
        ...
        if (sOpenedWlanAgps && sAGnssIface != nullptr) {
            sAGnssIface->data_conn_closed();
            sOpenedWlanAgps = false;
            ALOGI("%s: AGPS data conn on wlan0 closed", __func__);
            return;
        }
    }
...
Return<bool> AGnss::dataConnOpen(const hidl_string& apn, IAGnss::ApnIpType apnIpType) {
    ...
    (void)apn;
    if (!talkmanWlanIsUp()) {
        ALOGE("%s: wlan0 down, AGPS data conn failed", __func__);
        return (mAGnssIface->data_conn_failed() == 0);
    }
    int rc = mAGnssIface->data_conn_open_with_apn_ip_type(
            "wlan0", static_cast<uint16_t>(apnIpType));
    if (rc != 0) {
        ALOGE("%s: wlan0 data_conn_open failed %d", __func__, rc);
        return false;
    }
