...
struct AGnssRil : public IAGnssRil {
 ...
 private:
    void startWlanWatch();
    void wlanWatchLoop();
    void injectWlanNetworkState(bool up);
    static void* wlanWatchFn(void* arg);

    const AGpsRilInterface* mAGnssRilIface = nullptr;
    static const AGpsRilInterface* sAGpsRilIface;
    static sp<IAGnssRilCallback> sAGnssRilCbIface;
    static std::vector<std::unique_ptr<ThreadFuncArgs>> sThreadFuncArgsList;
    static bool sInterfaceExists;
    pthread_t mWlanThread = 0;
    std::atomic<bool> mWlanWatchRun;
    std::atomic<bool> mWlanWasUp;
    std::mutex mLock;
};
