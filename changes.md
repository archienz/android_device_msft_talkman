...
- Userspace in this tree: `talkman-rild-wait.sh` waits for HLOS ONLINE, runs two-phase `dsds`→`ss`, stops the phase-1 rild before starting Phone so stock RILJ `getService("slot1", true)` waits for the ss IRadio, and after bind + GET 0 issues one KEYCODE_WAKEUP so Phone SCREEN_STATE can re-queue NAS consider while `is_online=1`. Scores NAS **0x67** / `is_online` / `FORCE_NW_SEARCH` into kmsg. Does not send 0x67. `rild.legacy.rc` keeps `ril-daemon` disabled until that wait. qcril.db is a oneshot recover (not copy-every-boot). `persist.radio.force_nw_search=1`.
- Not proven: `mVoiceRegState=0` (`IN_SERVICE`). Do not mark a network Working. RSSI **99** is an empty QMI cache.
- Open hole: after the `ss` rild restart, Phone often does not call `setResponseFunctions`. `ITelephony.setRadioPower` (transaction 18) can return true anyway. The post-GET0 SCREEN_STATE keyevent is not proven on the telephone.
...
HAL is AOSP `hardware/qcom/audio/hal` (`msm8974` platform code, `PLATFORM_MSM8994`), kernel machine driver `sound/soc/msm/msm8994.c`, codec WCD9330 `msm8994-tomtom-snd-card`. Every item below was read in that source and checked on the telephone over ADB (HAL restart, `logcat`, `dmesg`, debugfs). Nothing here is a full build or a flash.

- **Headphone routing (fixed, `audio/mixer_paths.xml`, `audio/audio_platform_info.xml`).** The bullhead files put headphones on `SLIMBUS_5_RX` / `AIF3_PB` / `RX3`,`RX4`. The mmo kernel has no `SLIMBUS_5_RX` or `SLIMBUS_6_RX` back-end DAI link and no `SLIM_5_RX Channels`; `audio_route` logged `Control 'SLIMBUS_5_RX Audio Mixer MultiMedia1' doesn't exist`, the same for `SLIM_5_RX_Voice Mixer *`, `SLIM_5_RX Channels`, `HPHR DAC Switch`, enum `SLIM_5_RX` for `AUDIO_REF_EC_UL1 MUX`. So HPHL/HPHR never received PCM. Now headphones, line and TTY use `SLIMBUS_0_RX` / `AIF1_PB` / `RX1`,`RX2` (`SLIM_0_RX Channels` Two), the same as the CAF msm8994 reference and as the HAL's own `hw_interface_table[SND_DEVICE_OUT_HEADPHONES] = "SLIMBUS_0_RX"`. The `backend="headphones"` tags are removed: that tag selects `HEADPHONE_BACKEND`, whose mixer controls are hard-coded `SLIM_6_RX Format` / `SLIM_6_RX SampleRate` (`platform.c` `platform_backend_config_init`) and do not exist here; untagged devices use `DEFAULT_CODEC_BACKEND` = `SLIM_0_RX Format` / `SLIM_0_RX SampleRate`, which the kernel has. Speaker combos keep `speaker-and-headphones` so both `QUAT_MI2S_RX` and `SLIMBUS_0_RX` get the MultiMedia mixers. Pushed to `/vendor/etc` and HAL restarted: none of the headphone controls is reported missing any more; after the next boot `speaker` / `low-latency-playback speaker` applied and a 48 kHz WAV played on the loudspeaker. **Audible headphone test is pending**: no headset was present.
- **Hi-res headphones (not added).** The HAL can set 24-bit and 96/192 kHz only on `HEADPHONE_BACKEND` (`headset_is_config_supported`, `platform_set_backend_cfg` → `S24_LE`/`S24_3LE`, `KHZ_96`, `KHZ_192`), and only through the `SLIM_6_RX` controls. For `DEFAULT_CODEC_BACKEND` the HAL forces `CODEC_BACKEND_DEFAULT_BIT_WIDTH` (16) and 48 kHz (`platform_check_playback_backend_cfg`, `default:` branch). The kernel does expose `SLIM_0_RX Format` (`S16_LE`,`S24_LE`,`S24_3LE`) and `SLIM_0_RX SampleRate` (`KHZ_48`,`KHZ_96`,`KHZ_192`) with `msm_slim_0_rx_be_hw_params_fixup`, so the ADSP/codec side can do it; the HAL does not drive it. `check_input_parameters` also stops at 96000. No 24-bit or 96/192 kHz profile was added to `audio_policy_configuration.xml`; the HAL would refuse or resample it. What it takes: either a `SLIMBUS_6_RX` back end + `SLIM_6_RX Format`/`SampleRate` controls in `msm8994.c` (bullhead layout), or a HAL change that lets the default codec backend follow the stream. Both are code, not XML.
- **Compress offload (nothing added).** `audio_hw.c` `is_supported_format` accepts `AUDIO_FORMAT_MP3`, `AAC_LC`, `AAC_HE_V1`, `AAC_HE_V2`; `get_snd_codec_id` maps only `SND_AUDIOCODEC_MP3` / `SND_AUDIOCODEC_AAC`. No FLAC, ALAC, APE, Vorbis or WMA. `audio_policy_configuration.xml` already lists exactly those four; a FLAC profile would make AudioPolicy pick offload and the HAL return `-EINVAL`.
- **AudioFX (already in).** `vendor/lineage/config/common_mobile.mk` adds `AudioFX`; `lineage_talkman.mk` inherits `common_full_phone.mk`. On the telephone `pm list packages` shows `org.lineageos.audiofx`. `dumpsys media.audio_flinger` lists `bundle` (NXP Equalizer / Virtualizer / Dynamic Bass Boost / Volume), `reverb`, `proxy`, `offload_bundle` (`libqcompostprocbundle.so`), `visualizer_hw`, `loudness_enhancer`, `dynamics_processing`, `volume_listener`, `qcom_pre_processing` (Fluence AEC / NS). The `audio_effects.xml` offload UUIDs `2c4a8c24…`, `509a4498…`, `a0dac280…`, reverb `79a18026…`/`eb64ea04…`/`6987be09…`/`aa2bebf6…` match `hardware/qcom/audio/post_proc/*.c`. No change needed.
- **ANC headset (not possible with this HAL).** `hal/msm8974/platform.h` has no `SND_DEVICE_OUT_ANC_HEADSET`, `ANC_HANDSET` or `ANC_FB_HEADSET`; the `anc-handset`, `anc-headphones`, `voice-anc-*` paths already in `mixer_paths.xml` are never applied. The codec exposes `ANC Function`, `ANC Slot`, `ANC1 MUX`, `ANC1 FB MUX` and DAPM `ANC HPHL`/`ANC HPHR`, so the codec side exists. ANC also needs `acdb_loader_send_anc_cal` from the ACDB, which fails today (next item). No test possible; not claimed.
- **ACDB does not load (root cause found, not fixed).** `persist.audio.calfile0..7` in `system.prop` name `*_cal_8994.acdb` and `Codec_cal_8994.acdb`; `talkman-vendor.mk` ships `/system/etc/acdbdata/{Bluetooth,General,Global,Handset,Hdmi,Headset,Speaker}_cal.acdb` (bullhead import). So `acdb_loader_init_v2` returns `-1` before reading a file and the HAL logs `ACDB initialization failed`, `Error reading MBHC general config`, `Error: ACDB ANC returned = -1`. Every route runs without ADM/AFE calibration. Setting the real names on the telephone (props only, reverted) got further: the loader reached `AUDIO_ALLOCATE_CALIBRATION` on `/dev/msm_audio_cal` and the kernel answered `audio_cal_shared_ioctl: cal type size 16 is Invalid! Max is 0!`. Reason: bullhead's `libacdbloader.so` / `libaudcal.so` follow the bullhead `msm_audio_calibration.h`, which inserts `AFE_TOPOLOGY_CAL_TYPE`, `AFE_CUST_TOPOLOGY_CAL_TYPE`, `LSM_CUST_TOPOLOGY_CAL_TYPE`, `LSM_TOPOLOGY_CAL_TYPE` after `AFE_SIDETONE_CAL_TYPE`; the mmo kernel enum does not have them, so cal type 24 lands on `ADM_RTAC_INFO_CAL_TYPE` (size 0). With the loader half-initialised, the first `out_write` → `platform_send_gain_dep_cal` → `acdb_loader_send_gain_dep_cal` → `libaudcal GetMidPidCalibTable` dereferenced NULL and `android.hardware.audio.service` crash-looped (no sound at all). **Do not change the calfile names** until the kernel `include/uapi/linux/msm_audio_calibration.h` + `audio_cal_utils.c` (+ `q6afe.c`/`q6lsm.c` topology handlers) carry the bullhead cal-type order, or a `libacdbloader` built for the mmo ABI replaces the blob. Then ANC, gain-dependent cal and MBHC-from-ACDB become possible.
- **Loudspeaker + earpiece together (not possible in XML).** `platform.h` has `SND_DEVICE_OUT_SPEAKER_AND_HEADPHONES`, `_AND_LINE`, `_AND_HDMI`, `_AND_BT_A2DP`, `_AND_BT_SCO(_WB)`, `_AND_USB_HEADSET`; there is no speaker-and-handset device and AudioPolicy never asks for `SPEAKER|EARPIECE`. Doing it needs: a new `SND_DEVICE_OUT_SPEAKER_AND_HANDSET` in `platform.h` / `device_table` / `acdb_device_table` / `backend_tag_table`, a `platform_can_split_snd_device` entry, a selection rule in `platform_get_output_snd_device` (there is no `audio_devices_t` for it, so it would have to be a policy or a custom key-value), mixer paths `speaker-and-handset` and `<usecase> speaker-and-handset` (QUAT_MI2S_RX + SLIMBUS_0_RX with `RX1 → EAR`), and a policy `devicePort` if a real device type is chosen. Not done; no mixer path hacks.
- **Headset detection (kernel side, measured, no plug test).** `/dev/input`: `msm8994-tomtom-snd-card Headset Jack` (SW 0x02 headphone, 0x04 microphone, 0x06 line-out, 0x0e–0x11 unsupported/physical insert) and `msm8994-tomtom-snd-card Button Jack` (KEY 0x72/0x73/0xe2/0x104–0x107/0x246). `/sys/kernel/debug/wcd9xxx_mbhc`: `micb_mv = 2700mv`, `v_ins_hu = 2399mv`, `v_ins_h = 2398mv`, `v_b1_hu = 789mv`, `v_b1_h = 738mv` (thresholds computed, detection armed). MBHC runs on `def_codec_mbhc_cal()` from `msm8994.c` because the HAL's `wcdcal_hwdep` write is empty (`incorrect firmware size 0 for mbhc`, ACDB item above). DT 4-pole jack, inverted insert. A headset plug test by the owner is still needed.
- **Left as found (loudspeaker agent's area):** `AUDIO_REF_EC_UL1 MUX` has no `QUAT_MI2S_RX` value in this kernel (`ec_ref_rx` = `SLIM_RX`, `I2S_RX`, `PRI/SEC/TERT/QUAT_MI2S_TX`, `SEC_I2S_RX`, `PROXY_RX`), so `echo-reference speaker` does nothing; `RX4 DSM MUX` / `RX6 DSM MUX` and the `MultiMedia* Mixer MI2S_TX` capture controls do not exist either.
