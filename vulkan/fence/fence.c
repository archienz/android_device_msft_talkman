/*
 * talkman-vk-fence — Vulkan 1.0 empty command + VkFence on Lumia 950.
 *
...
static int empty_cmd_fence(VkPhysicalDevice gpu) {
    ...
    if (r != VK_SUCCESS) {
        talkman_err("empty command buffer record => %d (%s)", (int)r,
                    vk_result_str(r));
        goto out_pool;
    }
    talkman_log("empty cmd recorded (begin+end, no commands)");

    memset(&fci, 0, sizeof(fci));
    fci.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
    r = vkCreateFence(dev, &fci, NULL, &fence);
    talkman_log("vkCreateFence => %d (%s) fence=%p", (int)r, vk_result_str(r),
                (void*)fence);
    if (r != VK_SUCCESS)
        goto out_pool;
    ...
    talkman_log("vkQueueSubmit (empty + fence) => %d (%s)", (int)r,
                vk_result_str(r));
    if (r != VK_SUCCESS)
        goto out_fence;

    r = vkWaitForFences(dev, 1, &fence, VK_TRUE, UINT64_MAX);
    talkman_log("vkWaitForFences => %d (%s)", (int)r, vk_result_str(r));
    if (r != VK_SUCCESS)
        goto out_fence;

    status = vkGetFenceStatus(dev, fence);
    talkman_log("vkGetFenceStatus => %d (%s) %s", (int)status,
...
int main(void) {
    ...
    talkman_log("talkman-vk-fence start (Vulkan 1.0)");

    memset(&app, 0, sizeof(app));
    app.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    app.pApplicationName = "talkman-vk-fence";
    app.applicationVersion = 1;
    app.pEngineName = "talkman-vk-fence";
    app.engineVersion = 1;
    ...
    if (n == 0) {
        talkman_err("talkman-vk-fence: vkEnumeratePhysicalDevices n==0");
        vkDestroyInstance(inst, NULL);
        return 2;
    }
    ...
    free(gpus);
    vkDestroyInstance(inst, NULL);
    talkman_log("talkman-vk-fence done rc=%d%s", rc,
                rc == 0 ? " SIGNALED" : "");
    return rc;
}
