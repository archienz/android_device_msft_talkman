/*
 * talkman-vk-blit — GPU blit proof for Lumia 950 (MSM8992 / Adreno 418).
 *
...
static int create_opt_image(VkDevice dev, const VkPhysicalDeviceMemoryProperties* mp,
    ...
    if (r != VK_SUCCESS) {
        talkman_err("vkCreateImage => %d (%s)", (int)r, vk_result_str(r));
...
static int blit_and_readback(VkPhysicalDevice gpu) {
    ...
    if ((fp.optimalTilingFeatures & VK_FORMAT_FEATURE_BLIT_SRC_BIT) == 0 ||
        ...
        talkman_err("R8G8B8A8_UNORM missing blit src/dst (opt=0x%x)",
                    (unsigned)fp.optimalTilingFeatures);
        return 1;
    }
    ...
    if (create_opt_image(dev, &mp,
                         ...
                         &src, &src_mem) != 0) {
        talkman_err("src image create failed");
        goto out_src;
    }
    talkman_log("src image 64x64 R8G8B8A8_UNORM ok");

    if (create_opt_image(dev, &mp,
                         VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT,
                         &dst, &dst_mem) != 0) {
        talkman_err("dst image create failed");
        goto out_dst;
    }
    talkman_log("dst image 64x64 R8G8B8A8_UNORM ok");
    ...
    vkCmdClearColorImage(cmd, src, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clear,
                         1, &range);
    talkman_log("vkCmdClearColorImage src (1,0,0,1)");
    ...
    vkCmdBlitImage(cmd, src, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dst,
                   ...
                   VK_FILTER_NEAREST);
    talkman_log("vkCmdBlitImage src->dst NEAREST 64x64");
    ...
    talkman_log("vkQueueSubmit (clear+blit+copy) => %d (%s)", (int)r,
                vk_result_str(r));
    if (r != VK_SUCCESS)
        goto out_pool;
    ...
    talkman_log("dst first pixel RGBA=%u,%u,%u,%u", px[0], px[1], px[2], px[3]);
    if (px[0] > 0) {
        talkman_log("GPU blit ok (red channel %u)", px[0]);
        rc = 0;
    } else {
        talkman_err("red channel is 0; blit did not write dst");
        rc = 1;
    }
    vkUnmapMemory(dev, buf_mem);
    mapped = NULL;
...
int main(void) {
    ...
    talkman_log("talkman-vk-blit start (Vulkan 1.0)");

    memset(&app, 0, sizeof(app));
    app.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    app.pApplicationName = "talkman-vk-blit";
    app.applicationVersion = 1;
    app.pEngineName = "talkman-vk-blit";
    app.engineVersion = 1;
    ...
    if (r != VK_SUCCESS || inst == VK_NULL_HANDLE) {
        talkman_err("vkCreateInstance failed; not a software blit");
        return 1;
    }
    ...
    if (n == 0) {
        talkman_err("talkman-vk-blit: vkEnumeratePhysicalDevices returned 0");
        talkman_err("No Vulkan GPU. Not software-blitting.");
        vkDestroyInstance(inst, NULL);
        return 2;
    }
    ...
    free(gpus);
    vkDestroyInstance(inst, NULL);
    talkman_log("talkman-vk-blit done rc=%d", rc);
    return rc;
}
